        ...
        if($clusterCheck -ne $null){
            ...
            if($clusterServiceStatus -eq "Running"){
                ...
                if($nodes -eq $null){
                 ...
                 if($nodes -ne $null){
                    Write-Host "Clustered server diagnostic will be executed in clustered mode.." -ForegroundColor Yellow -BackgroundColor Black
                     $Clustered = $true
                }
                }
                else{
...
function Get-VMCurrentState
{
    Try
    {
        $BaselineValid = Get-ChildItem  C:\rs-pkgs\VMstatesBeginning.csv  -ErrorAction SilentlyContinue | Select-Object LastWriteTime,Name 
        If ($BaselineValid -eq $null)
        {
            Write-Host  "Baseline not present run menu option one" -ForegroundColor Yellow
            Write-Host  "or make sure baseline is in correct location C:\rs-pkgs\VMstatesBeginning.csv" -ForegroundColor Yellow
            Get-Menu 
        }
        If ($BaselineValid -ne $null)
        {
