...
function Update-RSEventInfo
{
    ...
    $LogNull = $LogArray.Add($AddtoLog2)
...
#Create new event log array to add entries to 
New-RSEventLog
...
Function Move-VMOwnerNode
{
    Try
    {
        ...
        foreach ($CSV in $CSVState)
        {
            ...
            if ($CSV.Name -match "Hype3_bronze_offsite")
            {
                if ($CSV.OwnerNode -match "826339-hyper-v3")
                {
                    #Write-Host "CSV is on correct node"
                    Update-RSEventInfo -AddtoLog "CSV $CSV.Name on preferred owner"
                }
                if ($CSV.OwnerNode -notmatch "826339-hyper-v3")
                {
                    #Write-Host "CSV is NOT on correct node"
                    Update-RSEventInfo -AddtoLog "CSV $CSV.Name moved to preferred owner"
                    Move-ClusterSharedVolume $CSV.Name -Node "826339-hyper-v3"
                }
            }
            <#
            OLD luns Decommed
            if ($CSV.Name -match "BronzeLUN_300GB")
            {
                if ($CSV.OwnerNode -match "795791-hyper-v1")
                {
                    Write-Host "CSV is on correct node"
                }
                if ($CSV.OwnerNode -notmatch "795791-hyper-v1")
                {
                    Write-Host "CSV is NOT on correct node"
                    Update-RSEventInfo -AddtoLog "CSV $CSV.Name moved to preferred owner"
                    Move-ClusterSharedVolume $CSV.Name -Node "795791-hyper-v1"
                }
            }
            if ($CSV.Name -match "BronzeLUN02_VM-DOCS")
            {
                if ($CSV.OwnerNode -match "795791-hyper-v1")
                {
                    Write-Host "CSV is on correct node"
            ...
            if ($CSV.Name -match "SilverLUN02_VHDS")
            {
                if ($CSV.OwnerNode -match "826339-hyper-v3")
                {
                    Write-Host "CSV is on correct node"
            ...
            #>
             if ($CSV.Name -match "hype1_bronze")
            {
                if ($CSV.OwnerNode -match "795791-hyper-v1")
                {
                    Update-RSEventInfo -AddtoLog "CSV $CSV.Name on preferred owner"
             ...
             if ($CSV.Name -match "hype1_silver")
            {
                if ($CSV.OwnerNode -match "795791-hyper-v1")
                {
                    Update-RSEventInfo -AddtoLog "CSV $CSV.Name on preferred owner"
             ...
             if ($CSV.Name -match "hype3_bronze")
            {
                if ($CSV.OwnerNode -match "826339-hyper-v3")
                {
                    Update-RSEventInfo -AddtoLog "CSV $CSV.Name on preferred owner"
             ...
             if ($CSV.Name -match "hype3_silver")
            {
                if ($CSV.OwnerNode -match "826339-hyper-v3")
                {
                    Update-RSEventInfo -AddtoLog "CSV $CSV.Name on preferred owner"
        ...
        $GetClusGroups = Get-ClusterGroup
                    ...
                    foreach ($VM in $VMMatch)
                    {
                            ...
                            if (-not $PrefOwner.OwnerNodes)
                            {
                                ...
                                Update-RSEventInfo -AddtoLog "$VM - No preferred owner Set"
                            ...
                            $CompareOwner = Compare-Object $VMCurrentOwner $PrefOwnerNode
                                if ($CompareOwner)
                                {
                                    ...
                                    #>
                                    $Script:VMCount ++
                                    Update-RSEventInfo -AddtoLog "$VM - Not on preferred owner"
                                ...
                                if (!$CompareOwner)
                                {
                                    ...
                                    Write-host $PrefOwnerNode 
                                    #>
                                    Update-RSEventInfo -AddtoLog "$VM - On preferred owner"
                                }
        ...
        Function Set-VMPreferredOwner
        {
            ...
            foreach ($VM in $VMMatch)
            {
                    ...
                    if (-not $PrefOwner.OwnerNodes)
                    {
                       $null = "$VM No preferred owner set $VMCurrentOwner" 
                    }
                    if($PrefOwner.OwnerNodes)
                    {
                    $CompareOwner = Compare-Object $VMCurrentOwner $PrefOwnerNode
                        if ($CompareOwner)
                        {
                            ...
                            Update-RSEventInfo -AddtoLog "Moving $VMToMove to $PrefOwner"
                            Get-ClusterGroup -Name $VMToMove  | Move-ClusterVirtualMachineRole -node  $PrefOwner -MigrationType Live
            ...
            Update-RSEventInfo -AddtoLog "VMs moved to preferred owner"
            $EventMessage = $LogArray | out-string
            Write-RSEventLog -EventLogInformation "$EventMessage" -Type Info
            }
            if ($Script:VMCount -eq 0)
            {
            Update-RSEventInfo -AddtoLog "All VMs Found to be on preferred owner"
            Update-RSEventInfo -AddtoLog "No Action Taken"
            $EventMessage = $LogArray | out-string
            Write-RSEventLog -EventLogInformation "$EventMessage" -Type Info
            }
        }
    ...
    {
        Return $_ 
        Update-RSEventInfo -AddtoLog "Unexpected Error running VM preferred owner script"
        Update-RSEventInfo -AddtoLog $_
        $EventMessage = $LogArray | out-string
        Write-RSEventLog -EventLogInformation "$EventMessage" -Type Error
    }
